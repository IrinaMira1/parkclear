...
export default function useParkingPrices(mapRef) {
    ...
    const enrichGarageWithPricing = useCallback(async (garage) => {
        ...
        if (now - lastFetchRef.current < RATE_LIMIT_MS) {
            await new Promise((resolve) => setTimeout(resolve, RATE_LIMIT_MS - (now - lastFetchRef.current)));
        }
